...
const report={project:'FLIP RADAR',version:'0.1.0',checked_at:new Date().toISOString(),node:process.version,
  status:'passed_local_checks_not_deployed',tests:{total:n('tests'),passed:n('pass'),failed:n('fail'),cancelled:n('cancelled'),skipped:n('skipped')},
  ...
  postgres_engine:'PGlite '+lock.packages['node_modules/@electric-sql/pglite'].version+' in memory; migration replayed twice',
  ...
  not_verified:['Import in user n8n instance','Live marketplace browsing','Production Chromium launch','Live model quality/API','Docker image build','Remote Postgres/Supabase connection','Telegram delivery','Deployment','Multi-worker load']};
...
await writeFile(root+'reports/VALIDATION_FR.md',`# FLIP RADAR — vérification locale\n\nDate UTC : ${report.checked_at}\n\n${report.tests.passed}/${report.tests.total} tests réussis, ${report.tests.failed} échec. ${report.javascript_files_syntax_checked} fichiers JavaScript vérifiés. Sept workflows n8n générés et inactifs. Le SELF TEST couvre ${self.tests_total} cas fictifs.\n\nPostgreSQL : ${report.postgres_engine}. Node : ${report.node}. Dépendances : pg ${report.configured_dependencies.pg}, Playwright ${report.configured_dependencies.playwright}.\n\nAucun appel à une marketplace ou un modèle distant, aucun Telegram ni achat exécuté. Les serveurs HTTP utilisés par les tests sont locaux.\n\nLa migration et les transactions ont été réellement exécutées dans PGlite ; le navigateur et le modèle sont simulés. Voir TEST_RESULTS.tap pour le détail et SELF_TEST_RESULT.json pour l'exemple fictif.\n\nRestent à vérifier : import dans ton n8n, Docker/Chromium en production, source réelle, modèle, base distante, Telegram, déploiement et charge. Le système n'est pas déployé.\n`);
const manifest=[];
...
await writeFile(root+'reports/SOURCE_MANIFEST.json',JSON.stringify({version:'0.1.0',files:manifest},null,2)+'\n');
