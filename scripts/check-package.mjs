...
const report={project:'FLIP RADAR',version:'0.2.0',checked_at:new Date().toISOString(),node:process.version,
  ...
  not_verified:['Workflow 23 import in user n8n instance','DNID import through deployed worker','Live marketplace browsing','Production Chromium launch','Live model quality/API','Docker image build','Supabase migration 002','Telegram delivery','Version 0.2 deployment','Multi-worker load']};
...
await writeFile(root+'reports/SOURCE_MANIFEST.json',JSON.stringify({version:'0.2.0',files:manifest},null,2)+'\n');
